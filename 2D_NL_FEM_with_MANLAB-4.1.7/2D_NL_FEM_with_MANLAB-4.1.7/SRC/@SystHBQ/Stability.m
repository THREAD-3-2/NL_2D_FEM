...
if sum(isinf(eig_values))>0
    disp('SystHBQ/Stability.m : Some eigenvalues are infinite. Stability results might be wrong.');
end
...
if (length(ind2) < nz/10 || length(ind2) > nz)
    disp('SystHBQ/Stability.m : Floquet Exponents not converged. Stability results may be wrong.');
end
