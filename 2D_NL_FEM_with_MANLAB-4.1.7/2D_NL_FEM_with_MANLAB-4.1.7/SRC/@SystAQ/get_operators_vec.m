function [sys] = get_operators_vec(sys,equations,nR,nU,iR,iU)
%
...
vdQ = cell(nU,1); idQ = cell(nU,1); jdQ = cell(nU,1); kdQ = cell(nU,1);
    ...
    %% constant operator C
    valc = equations(sys,Zcolnul,Zcolnul);
    valc(ind_diff) = 0;
    valc(abs(valc)<eps)=0;
    [iC,~,vC] = find(valc);
    ...
    valL = (equations(sys,Id,Znul)-equations(sys,-Id,Znul))/2;
    [~,valdL] = equations(sys,Znul,Id);
        
    for j=1:nU
        
        valQ = (equations(sys,Id+Id(:,j),Znul)-equations(sys,Id-Id(:,j),Znul))/2;
        [~,valdQ] = equations(sys,repmat(Id(:,j),1,nU),Id);
    ...
    [Rtest,~] = equations(sys,ones(nU,1)/2,Zcolnul);
    for i=1:ndiff
