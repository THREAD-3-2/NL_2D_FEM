function [Rf,dRa,Forcing] = equations_vector_nnm(sys, t, zf, dzf, d2zf)
...
active_dof = sys.model.boundary.active_dof;
prescribed_dof = sys.parameter.model.boundary.prescribed_dof;

%%% parameters of the system
number_nodes = sys.parameter.model.mesh.number_nodes;
number_elements = sys.parameters.model.mesh.number_elements;
n_var = sys.nz/2; % location of first variable u

E = sys.parameters.model.prop.young_mod;
A = sys.parameters.model.prop.area;
I = sys.parameters.model.prop.I;
G = sys.parameters.model.prop.shear_mod;
k = sys.parameters.model.prop.coeff_shear_k; % shear coefficient
% 
...
% C_global = 2*epsilon*24.67*M_global;
%C_global = epsilon*M_global;
...
Residual = -f_int_global - M_global*dv_global - lambda.*v_global;
%% Residues
...
% Forcing(:,2*n_var - 1) = 1*cos(t);
% Forcing(:,n_var + 4) = 50*cos(t);
end