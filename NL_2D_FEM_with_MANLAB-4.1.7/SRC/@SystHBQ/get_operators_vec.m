...
H = sys.H;
Z = sparse(nz_tot+1,nz_tot);
Id = speye(nz_tot+1);
lambda=repmat(Id(:,end),1,nz_tot);
Id = Id(:,1:end-1);
...
[~,dRtest] = feval(equations,cell_argin_eq{:},randn(nz_tot+1,1),randn(nz_tot+1,1),Z(:,1));
ind_diff = find(dRtest)+nz;

%% constant operators C0 and C1
valc0 = feval(equations,cell_argin_eq{:},Z(:,1),Z(:,1),Z(:,1));
valc1 = feval(fun_eq_bilin,lambda(:,1),Z(:,1),Z(:,1),Z(:,1));
valc2 = (feval(fun_eq_bilin,lambda(:,1),lambda(:,1),Z(:,1),Z(:,1)) - valc1)/2;
valc0(ind_diff) = 0;
valc1(ind_diff) = 0;
valc2(ind_diff) = 0;
...
val0 = feval(fun_eq_bilin,Id,Z,Z,Z);
val1 = feval(fun_eq_bilin,Id,lambda,Z,Z);
valdd = feval(equations,cell_argin_eq{:},Z,Z,Id);
[vald,valdl] = feval(equations,cell_argin_eq{:},Z,Id,Z);
vald1 = feval(equations,cell_argin_eq{:},lambda,Id,Z);
...
for j=1:nz_tot
    ...
    valq = feval(fun_eq_bilin,repmat(Id(:,j),1,nz_tot),Id,Z,Z);
    [~,valdq] = feval(equations,cell_argin_eq{:},repmat(Id(:,j),1,nz_tot),Id,Z);
...
cell_argin_eq{2} = t;
[~,~,forced_terms0] = feval(equations,sys,t,Z(:,1),Z(:,1),Z(:,1));
[~,forced_terms1] = feval(fun_eq_bilin,lambda(:,1),Z(:,1),Z(:,1),Z(:,1));
[~,forced_terms2] = feval(fun_eq_bilin,lambda(:,1),lambda(:,1),Z(:,1),Z(:,1));
