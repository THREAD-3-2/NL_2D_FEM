...
[U0, omega0, lambda0, qs_full, freq, shape, idx, amp] = model.initialise_MAN_computation(sys, type, target_mode);
...
dispvars = [sys.getcoord('omega') sys.getcoord('cos',(N-2)*3+1,1);  % -> sys.getcoord('sin', dof_index, harmonic_number)
            sys.getcoord('omega') sys.getcoord('sin',(N-2)*3+1,1);
            sys.getcoord('omega') sys.getcoord('cos',(N-2)*3+2,1);
            sys.getcoord('omega') sys.getcoord('sin',(N-2)*3+2,1)];
