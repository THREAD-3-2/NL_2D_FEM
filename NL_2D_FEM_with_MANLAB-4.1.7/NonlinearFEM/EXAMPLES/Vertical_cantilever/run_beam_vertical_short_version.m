...
mds = vertical_beam_problem();
% create a new NL_2D_FEM model
model = NL_2D_FEM; 
% set up the model using the data structure
model = model.set_model_from_mds(mds);
% buckling analysis
lambda = [linspace(0.1, 7, 10), linspace(7.1, 8, 20), linspace(-0.1, -10, 5) ];
[frq_list, Lambda_range, N_list, q_list] = model.buckling_analysis(lambda);
plot(7.8373,0,'o');
legend('FEM Analysis', 'Theoretical bucling Load (7.8373)')

%% MANLAB LAUNCHING SEQUENCE
global U Section Diagram   % Global variables to export point from the diagram in GUI
%% MANLAB INPUTS
H = 30;          % number of harmonics for the Fourier series   
