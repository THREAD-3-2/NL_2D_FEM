function mds = horizontal_rope_problem()
%% Input definition
...
mds.geom.discretisation = [20]; % line 1 discretized with 5 elements
...
g = 9810;
epsi = I/(S*L^2);
wo = sqrt(E*I/rho/S/L^4);
ag = g/L/wo^2; 
mds.prop.S = 1; mds.prop.I = epsi*2e-2; mds.prop.rho = 1; mds.prop.E = 1/epsi;
...
mds.force.static.static_ponctual_force_node_list{1} = struct('node', 2,'dof', [2],'amplitude', [ag/2] ); % Fx = -1 and Fy = 0 at node 2
% distrubuted static forces
mds.force.static.static_distributed_force_amplitude = [-ag];
mds.force.static.static_distributed_force_direction = [ 2];
% ponctual periodic forces
mds.force.periodic.periodic_ponctual_force_node_list{1} = struct('node', 2,'dof', [2],'amplitude', [0], 'harmonic', [1] ); % complex amplitude f = re(amp) cos + im(amp) sin
