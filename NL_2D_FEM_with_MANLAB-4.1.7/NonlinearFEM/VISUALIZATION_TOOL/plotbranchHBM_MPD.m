...
function Hand = plotbranchHBM_MPD(sys,Sec_Diag,Idisp,Hdisp,bifpara_str,LineWidth,Color)
...
%         bifpara = Section.Upp(ind_bifpara,:)
        bifpara = Section.Upp(ind_bifpara,:)/sys.parameters.natural_puls(sys.parameters.mode);
        ...
        for ii=1:Nb
            ...
            %%% Test if there is a bifurcation in the Section
            if strcmp(Section.Eigen.type,'nothing')
                hand = plot(bifpara,Hampl,st1,'Color',Color,'linewidth',LineWidth);
                Hand = [Hand;hand];
            else
                Hampl_stab   = sqrt(Section.Ustab(Icos,:).^2+Section.Ustab(Isin,:).^2);
                Hampl = [Hampl(1:ind_change-1),Hampl_stab,Hampl(ind_change:end)];
                
                hand = plot(bifpara(1:ind_change),Hampl(1:ind_change),st1,'Color',Color,'linewidth',LineWidth);
                Hand = [Hand;hand];
                hand = plot(bifpara(ind_change:end),Hampl(ind_change:end),st2,'Color',Color,'linewidth',LineWidth);
                Hand = [Hand;hand];
                hand = plot(bifpara(ind_change),Hampl(ind_change),'.','Color',Color,'linewidth',LineWidth);
                Hand = [Hand;hand];
                
            end
            
            Handleg(ii) = plot(NaN,NaN,'-','Color',Color,'linewidth',LineWidth);
