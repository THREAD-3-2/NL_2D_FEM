function plot_deformed_mesh(mesh,q_global, fig, marker)
...
maxX = max(mesh.nodes(:,2));
maxY = max(mesh.nodes(:,3));
minX = min(mesh.nodes(:,2));
minY = min(mesh.nodes(:,3));

for i = 1:mesh.number_elements
    nodeA = mesh.connect(i,2);  % first node of element
    nodeB = mesh.connect(i,3);  % second node of element
    ...
    P1 = mesh.nodes(nodeA, 2:3) + q_e(1:2)'; % position of node A after deformation
    P2 = mesh.nodes(nodeB, 2:3) + q_e(4:5)'; % position of node B after deformation
